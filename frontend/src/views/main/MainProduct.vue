    ...
    <div v-if="events" class="table-container">
      <div class="btn-background">
        <div class="event-btn ml-3 mt-3 mb-3">
          ...
          <button type="button" class="btn btn-info ml-3" @click="onCreate()">추천 상품</button>
        </div>
      </div>
      <table class="table">
        <thead class="">
...
export default {
  ...
  methods: {
    ...
    onCreate() {
			this.$router.push({name: 'MainProductCreate', params: {method: 'create'}}, () => {})
    },
    onUpdate(id) {
			this.$router.push({name: 'MainProductUpdate', params: {method: 'update', id: id}}, () => {})
		},
    checkAll(events) {
